...
def differentiable_kabsch(
    ...
    def _has_zero_singular_value(singular_values):
        return singular_values.min() < 1e-3
    # Generate all (sigma_i)^2 - (sigma_j)^2 and check if any are close to zero
    def _has_degenerate_singular_values(singular_values):
        pairwise = singular_values**2 - (singular_values**2).view(num_coords, 1)
        pairwise = pairwise + torch.eye(num_coords, device=device)
        return pairwise.abs().min() < 1e-2
    num_it = 0
