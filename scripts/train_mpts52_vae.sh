...
RUN_NAME="${RUN_NAME:-mpts52_transformer_vae_d512_z8}"
...
exec "$PYTHON_BIN" src/train_autoencoder.py \
  data=mpts_52 encoder=transformer decoder=transformer \
  ...
  autoencoder_module.loss_weights.loss_atom_types.mp20=1.0 \
  autoencoder_module.loss_weights.loss_lengths.mp20=1.0 \
  autoencoder_module.loss_weights.loss_angles.mp20=10.0 \
  autoencoder_module.loss_weights.loss_frac_coords.mp20=10.0 \
